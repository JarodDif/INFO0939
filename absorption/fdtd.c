  ...
  for (int p = 0; p < numnodesz; p++) {
    for (int n = 0; n < numnodesy; n++) {
      for (int m = 0; m < numnodesx; m++) {
        ...
        dvx -= m > 0 ? GETVALUE(simdata->vxold, m - 1, n, p) : GETVALUE(simdata->vxold, m, n, p);
        dvy -= n > 0 ? GETVALUE(simdata->vyold, m, n - 1, p) : GETVALUE(simdata->vxold, m, n, p);
        dvz -= p > 0 ? GETVALUE(simdata->vzold, m, n, p - 1) : GETVALUE(simdata->vxold, m, n, p);
