...
#define PNUMNODESTOT(pdat) ((size_t) (pdat)->grid.lm.n * (pdat)->grid.ln.n * (pdat)->grid.lp.n)
...
#define PINDEX3D(pdat, mbar, nbar, pbar) \
  ((size_t) PNUMNODESX((pdat)) * PNUMNODESY((pdat)) * (pbar) + PNUMNODESX((pdat)) * (nbar) + (mbar))

#define PROCESS_GETVALUE_INSIDE(pdat, mbar, nbar, pbar)                        \
  ((pdat)->vals[PINDEX3D(pdat, mbar, nbar, pbar)])

#define PROCESS_SETVALUE_INSIDE(pdat, mbar, nbar, pbar, val)                   \
  ((pdat)->vals[PINDEX3D(pdat, mbar, nbar, pbar)] = (val))
