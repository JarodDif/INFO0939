...
#pragma omp declare mapper(data_t data) \
              map(to:data) \
              map(   data.vals[0:NUMNODESTOT(data.grid)])

#pragma omp declare mapper(simulation_data_t simdata) \
              map(to:simdata) \
              map(to:simdata.c[0:1]) map(to:simdata.rho[0:1]) map(simdata.rhohalf[0:1]) \
              map(   simdata.pold[0:1]) map(to:simdata.pnew[0:1]) \
              map(   simdata.vxold[0:1]) map(to:simdata.vxnew[0:1]) \
              map(   simdata.vyold[0:1]) map(to:simdata.vynew[0:1]) \
              map(   simdata.vzold[0:1]) map(to:simdata.vznew[0:1])
...
void swap_timesteps(simulation_data_t *simdata) {
  ...
  #pragma omp target teams distribute map(to:numnodesx, numnodesy, numnodesz)
  for (int p = 0; p < numnodesz; p++) {
    #pragma omp parallel for collapse(2) 
    for (int n = 0; n < numnodesy; n++) {
      for (int m = 0; m < numnodesx; m++) {
