...
void update_pressure(process_simulation_data_t *psimdata) {
  ...
  for (p = 0; p < pnumnodesz; p++) {
    for (n = 0; n < pnumnodesy; n++){
      psimdata->buffer_vx[p * pnumnodesy + n] = process_getvalue(psimdata->vxold, ENDM(pold), n + STARTN(pold), p + STARTP(pold));
    }
  }
  for (p = 0; p < pnumnodesz; p++) {
    for (m = 0; m < pnumnodesx; m++){
      psimdata->buffer_vy[p * pnumnodesx + m] = process_getvalue(psimdata->vyold, m + STARTM(pold), ENDN(pold), p + STARTP(pold));
    }
  }
  for (n = 0; n < pnumnodesy; n++) {
    for (m = 0; m < pnumnodesx; m++){
      psimdata->buffer_vy[n * pnumnodesx + m] = process_getvalue(psimdata->vzold, m + STARTM(pold), n + STARTN(pold), ENDP(pold));
    }
  }

  MPI_Isend(psimdata->buffer_vx, pnumnodesy*pnumnodesz, MPI_DOUBLE, neighbors[RIGHT], SEND_VX, cart_comm, &request_send[0]);
  MPI_Isend(psimdata->buffer_vy, pnumnodesx*pnumnodesz, MPI_DOUBLE, neighbors[BACK ], SEND_VY, cart_comm, &request_send[1]);
  MPI_Isend(psimdata->buffer_vz, pnumnodesx*pnumnodesy, MPI_DOUBLE, neighbors[UP   ], SEND_VZ, cart_comm, &request_send[2]);
